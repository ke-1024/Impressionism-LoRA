...
- 基础模型：Stable Diffusion 1.5（推荐`v1-5-pruned-emaonly.safetensors`版本）；
...
1. 模型安装：
        将Stable Diffusion 1.5基础模型放入WebUI的`models/Stable-diffusion/`目录；

2. 将本项目LoRA模型文件放入`models/Lora/`目录，重启WebUI即可加载。

3. 生成参数示例：
        提示词（Prompt）：`water lilies in a pond, morning light, soft reflection, impressionist style <lora:Impressionism-LoRA-v1.0:0.8>`；

4. 反向提示词（Negative Prompt）：`low resolution, blurry, sharp edges, photorealistic, text, watermark`；

5. 采样配置：采样器DPM++ 2M Karras，推理步数20-30，分辨率512x512/768x512，CFG Scale 7-9；

6. 风格强度调整：通过LoRA权重参数（0.6-1.0）控制风格化程度，权重越高风格特征越显著。
...
训练数据来源于Kaggle平台的Painter by Numbers项目，从中筛选200张高质量印象派作品，涵盖风景、人像、静物三大核心类别。该数据集的图像主要源自[WikiArt.org](http://WikiArt.org)，所有素材均受版权保护，本项目仅将其用于数据挖掘与模型训练，符合合理使用原则。
...
- 训练日志：详细参数与过程记录见`docs/training_log.txt`。
...
## 五、许可证声明
本项目采用MIT许可证授权，允许非商业与商业场景下的使用、修改及分发。使用者需遵守相关版权规定，不得将模型用于侵权或违规场景，模型开发者不对使用过程中产生的任何损失承担责任。
